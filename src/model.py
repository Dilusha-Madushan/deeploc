...
# Updated pos_weights_bce for handling class imbalance
pos_weights_bce = torch.tensor([1, 1, 1, 3, 2.3, 4, 9.5, 4.5, 6.6, 7.7, 32])


# Updated focal_loss function for handling class imbalance
def focal_loss(input, target, gamma=1):
    bceloss = F.binary_cross_entropy_with_logits(input, target, pos_weight=pos_weights_bce.to(input.device),
                                                 reduction="none")
...
class AttentionHead(nn.Module):
    ...
    def forward(self, x, np_mask, lengths):
        ...
        for i in range(mul.size(0)):
            # (1, L) -> (1, 1, L) -> (1, L) -> (1, L, 1)
            mul_score_list.append(F.pad(smooth_tensor_1d(mul[i, :lengths[i], 0].unsqueeze(0), 2).unsqueeze(0),
                                        (0, mul.size(1) - lengths[i]), "constant").squeeze(0))
